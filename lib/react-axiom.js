	  ...
	  _createClass(Store, [{
	    ...
	    value: function toSerializable(data, store) {
	      ...
	      if (data instanceof _Model3.default) {
	        ...
	        if (!store[_constructor.name][data._id]) {
	          store[_constructor.name][data._id] = this.toSerializableState(state, store);
	        }
	    ...
	    value: function fromSerializable(data, models, newModels) {
	      ...
	      if ((0, _isPlainObject2.default)(data)) {
	        if (data._constructor) {
	          ...
	          delete data._id;
	          var state = this.fromSerializable(models[name][id], models, newModels);
